...
\frametitle{Model Checking}
  Over finite-state and infinite-state systems
  \pause
...
\begin{frame}
\frametitle{Algorithms}
    SAT-Based algorithms \pause (LTL only)
    \begin{itemize}
      \pause 
      \item BMC
        \begin{itemize}
          \pause 
          \item Try to find a counter example with depth up to $k$
          \pause 
    ...
    \end{itemize}
  \pause 
Faster than BDD-based algorithm
\end{frame}
...
\begin{frame}
  ...
  \pause 
    \item SMT-Based algorithms 
      \begin{itemize}
        \item Implement BMC, K-liveness
      \end{itemize}
    \pause
    \item SMT generalizes SAT
    \pause
    \item go\_msat, msat\_check\_ltlspec\_bmc 
 \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    \item nuXmv is a powerful model checker 
    \item supporting finite and infinite state models
    \item only available as a command line tool
    \item we would not recommend you to use it due to its documentation 
  \end{itemize}
...
\printbibliography
\phantom{\cite{DBLP:conf/cav/CavadaCDGMMMRT14}}
